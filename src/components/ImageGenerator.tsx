...
import { getActiveImageModels } from '../lib/modelSync'
...
export function ImageGenerator({ profile, profiles = [], activeProfileId = '', onProfileSwitch, saveCampaign, onOpenKeys }: Props) {
  const [model, setModel] = useState<ImageModel>(() => {
    const active = getActiveImageModels()
    return active['nano-banana'] ? 'nano-banana' : (Object.keys(active)[0] as ImageModel)
  })
  ...
  const activeImageModels = getActiveImageModels()
...
Return ONLY the prompt, nothing else.`
        ...
        finalPrompt = await generateText(promptInstruction, 'llama-3.3-70b')
      ...
      <div className="gen-layout" style={{ gridTemplateColumns: '320px 1fr' }}>
        ...
        <div className="gen-controls">
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            {(Object.entries(activeImageModels) as [ImageModel, typeof IMAGE_MODELS[ImageModel]][]).map(([key, m]) => (
              <div key={key}
